...
function _buildPool(count, geo, isSmoke) {
  ...
  function _acquireSmoke() {
    const len = smokePool.length;
    for (let i = 0; i < len; i++) {
      const idx = (_smokeHead + i) % len;
      if (!smokePool[idx].active) {
        _smokeHead = (idx + 1) % len;
        return smokePool[idx];
      }
    }
  ...
  // ── Builder crit fire jet — rocket bursts from building footprint ────────
  function spawnBuildingFireJet(bx, bz, bw, bd) {
    const JETS = 7, PER_JET = 14;
    for (let j = 0; j < JETS; j++) {
      const ox = bx + (Math.random() - 0.5) * bw * 0.9;
      const oz = bz + (Math.random() - 0.5) * bd * 0.9;
      for (let k = 0; k < PER_JET; k++) {
        let slot = -1;
        for (let i = 0; i < firePool.length; i++) {
          const idx = (_fireHead + i) % firePool.length;
        ...
        p.vy        = 9 + Math.random() * 12; // strong rocket upward
        p.vz        = (Math.random() - 0.5) * 3.0;
        p.initScale = 0.6 + Math.random() * 1.4;
        p.mesh.position.set(
  ...
  function spawnBuildingDestroyDebris(bx, bCenterY, bz, bw, bh, bd, color, crit = false) {
    const BASE  = Math.max(12, Math.round(bw * bd * 6)); // ~12 for 1×1, ~54 for 3×3
    const COUNT = crit ? BASE * 2 : BASE;
    while (deathParticles.length + COUNT > _DEATH_PARTICLE_MAX) {
      const old = deathParticles.shift();
      scene.remove(old.mesh); old.mesh.geometry.dispose(); old.mesh.material.dispose();
    }
    for (let k = 0; k < COUNT; k++) {
      ...
      mesh.castShadow = true;
      const theta  = Math.random() * Math.PI * 2;
      // Crit: stronger upward bias and speed
      const upBias = crit ? 0.55 + Math.random() * 0.45 : 0.3 + Math.random() * 0.7;
      const speed  = crit ? 8  + Math.random() * 10     : 4   + Math.random() * 7;
      ...
      deathParticles.push({
        ...
        vy: upBias * speed * 0.85 + 2 + (crit ? 8 : 0), // extra upward kick on crit
  ...
  function updateEffects(elapsedSec) {
    ...
    players.forEach((player, i) => {
      ...
      // ── Critical hit effect ───────────────────────────────────────────────
      const _critPool = [];
      const _CRIT_FLASH_GEO = new THREE.SphereGeometry(0.5, 10, 8);
      const _CRIT_PART_GEO  = new THREE.SphereGeometry(0.08, 6, 5);
      const _CRIT_RAY_GEO   = new THREE.BoxGeometry(0.07, 0.07, 0.55);

      // Shared canvas texture for "CRITICAL" label (text never changes)
      const _critTextTex = (() => {
        const cw = 320, ch = 72;
        const canvas = document.createElement('canvas');
        canvas.width = cw; canvas.height = ch;
        const ctx = canvas.getContext('2d');
        ctx.font = 'bold italic 52px monospace';
        ...
        ctx.lineWidth = 7;
        ctx.lineJoin = 'round';
        ctx.strokeText('CRITICAL', cw / 2, ch / 2);
        // Yellow fill
        ctx.fillStyle = '#ffdd00';
        ctx.fillText('CRITICAL', cw / 2, ch / 2);
        const tex = new THREE.CanvasTexture(canvas);
        return tex;
      })();

      function spawnCriticalHit(wx, wy, wz) {
        // 1. White flash sphere — expands outward and vanishes
        {
          const mesh = new THREE.Mesh(
            _CRIT_FLASH_GEO,
          ...
          mesh.position.set(wx, wy, wz);
          scene.add(mesh);
          _critPool.push({ mesh, life: 0.14, maxLife: 0.14, type: 'flash', vx: 0, vy: 0, vz: 0 });
        }

        // 2. Orange inner sphere — slightly slower fade
        {
          const mesh = new THREE.Mesh(
            _CRIT_FLASH_GEO,
            new THREE.MeshBasicMaterial({ color: 0xff8800, transparent: true, opacity: 0.75, depthWrite: false })
          );
          mesh.position.set(wx, wy, wz);
          mesh.scale.setScalar(0.55);
          scene.add(mesh);
          _critPool.push({ mesh, life: 0.22, maxLife: 0.22, type: 'inner', vx: 0, vy: 0, vz: 0 });
        }

        // 3. Star rays — 8 elongated boxes radiating outward in XZ plane
        const RAY_COUNT = 8;
        for (let i = 0; i < RAY_COUNT; i++) {
          const angle = (i / RAY_COUNT) * Math.PI * 2 + Math.random() * 0.2;
          const color = i % 2 === 0 ? 0xffdd00 : 0xff6600;
          const mesh = new THREE.Mesh(
            _CRIT_RAY_GEO,
          ...
          const vz = Math.cos(angle) * spd;
          mesh.position.set(wx + vx * 0.05, wy, wz + vz * 0.05);
          mesh.rotation.y = -angle;
          scene.add(mesh);
          _critPool.push({ mesh, life: 0.26, maxLife: 0.26, type: 'ray', vx, vy: 0, vz });
        }

        // 4. "CRITICAL" text sprite — floats up and fades out
        {
          const mat  = new THREE.SpriteMaterial({ map: _critTextTex, transparent: true, opacity: 0, depthTest: false });
          const sprite = new THREE.Sprite(mat);
          sprite.scale.set(2.8, 0.63, 1); // aspect ratio matches canvas 320:72
          sprite.position.set(wx, wy + 0.4, wz);
          sprite.renderOrder = 999;
          scene.add(sprite);
          const maxLife = 1.4;
          _critPool.push({ mesh: sprite, life: maxLife, maxLife, type: 'text', vx: 0, vy: 1.8, vz: 0 });
        }

        // 5. Bright particle burst — spheres flying in all directions
        const colors = [0xffffff, 0xffee44, 0xff8800, 0xffcc00];
        for (let i = 0; i < 14; i++) {
          const color = colors[Math.floor(Math.random() * colors.length)];
          const mesh = new THREE.Mesh(
            _CRIT_PART_GEO,
          ...
          const theta = Math.random() * Math.PI * 2;
          const phi   = (0.3 + Math.random() * 0.7) * Math.PI; // mostly outward, not straight up/down
          const spd   = 4.5 + Math.random() * 5.5;
          const vx = Math.sin(phi) * Math.cos(theta) * spd;
          const vy = Math.abs(Math.cos(phi)) * spd * 0.6 + 1.5; // upward bias
          const vz = Math.sin(phi) * Math.sin(theta) * spd;
          const scale = 0.6 + Math.random() * 0.8;
          mesh.scale.setScalar(scale);
          mesh.position.set(wx, wy, wz);
          scene.add(mesh);
          const maxLife = 0.2 + Math.random() * 0.18;
          _critPool.push({ mesh, life: maxLife, maxLife, type: 'particle', vx, vy, vz });
        }
      }

      function _updateCritEffects(dt) {
        for (let i = _critPool.length - 1; i >= 0; i--) {
          const c = _critPool[i];
          c.life -= dt;
          if (c.life <= 0) {
            scene.remove(c.mesh);
            c.mesh.material.dispose();
            _critPool.splice(i, 1);
            continue;
          }
          const frac = c.life / c.maxLife; // 1 → 0
          if (c.type === 'flash') {
            c.mesh.scale.setScalar(1 + (1 - frac) * 3.0);
            c.mesh.material.opacity = frac * 0.85;
          } else if (c.type === 'inner') {
            c.mesh.scale.setScalar(0.55 + (1 - frac) * 1.8);
            c.mesh.material.opacity = frac * 0.7;
          } else if (c.type === 'ray') {
            c.mesh.position.x += c.vx * dt;
            c.mesh.position.z += c.vz * dt;
            c.mesh.scale.z = 1 + (1 - frac) * 1.5; // stretch as it flies out
            c.mesh.material.opacity = frac * frac;
          } else if (c.type === 'text') {
            c.mesh.position.y += c.vy * dt;
            // Quick pop-in (first 10% of life), hold, then fade out (last 20%)
            const progress = 1 - frac; // 0 → 1
            const scaleT   = Math.min(1, progress / 0.1);
            const fadeT    = Math.max(0, (frac - 0.8) / 0.2);
            c.mesh.material.opacity = scaleT * fadeT;
            const s = 0.5 + scaleT * 0.5; // 0.5 → 1.0
            c.mesh.scale.set(2.8 * s, 0.63 * s, 1);
          } else { // particle
            c.mesh.position.x += c.vx * dt;
            c.mesh.position.y += c.vy * dt;
            c.mesh.position.z += c.vz * dt;
      ...
      // ── Blast-away smoke (Killer crit fly phase) ──────────────────────────
      // Emits dense white puffs at the flying character's current position.
      // Called every frame during the fly phase — uses the shared smoke pool.
      function emitBlastSmoke(x, y, z) {
        for (let n = 0; n < 5; n++) {
          const p = _acquireSmoke();
          if (!p) break;
          p.active    = true;
          ...
          p.initScale = 2.0 + Math.random() * 1.5; // large dense puffs
          p.mesh.position.set(
      ...
      function resetEffects() {
        ...
        _critPool.forEach(c => { scene.remove(c.mesh); c.mesh.material.dispose(); });
        _critPool.length = 0;
      }
