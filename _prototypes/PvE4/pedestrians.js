...
function _spawnFromTop() {
  ...
  const z = -AREA - 4 - Math.random() * 3;  // 畫面外 4–7 格，走進來才出現
...
function _spawnAtPos(wx, wz) {
  ...
  const z = clamp(wz + (Math.random() - 0.5) * 1.8, -AREA, DAMAGE_Z - 1.0);
  const ped = _makePedAt(x, z);
  if (ped) pedestrians.push(ped);
}
...
function spawnPedsAtBuilding(bx, bz, hpMax) {
  ...
  const count = Math.max(1, Math.round(hpMax / 10));
...
function updatePedestrians(dt, dtSec) {
  ...
  const scrollStep = paused ? 0 : _activeScrollSpeed * dt;
