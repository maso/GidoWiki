...
function updateItems(dtSec) {
  ...
  const scrollStep = gamePaused ? 0 : _activeScrollSpeed * dtSec * 60;

  // ── Spawn rings ───────────────────────────────────────────────────────
  for (let i = _itemRings.length - 1; i >= 0; i--) {
    const r = _itemRings[i];
    r.mesh.position.z += scrollStep;
    if (!gamePaused) {
      r.t += dtSec;
  ...
  if (!gamePaused) {
    for (let i = _sparks.length - 1; i >= 0; i--) {
      ...
      s.mesh.position.z += s.vz * dtSec + scrollStep;
      s.life -= dtSec;
  ...
  if (!gamePaused) {
    for (let i = _iconPops.length - 1; i >= 0; i--) {
      ...
      h.sprite.position.z += scrollStep;
      if (frac >= 1) {
  ...
  for (let i = items.length - 1; i >= 0; i--) {
    ...
    item.shadowMesh.position.z += scrollStep;

    // Remove once scrolled off-screen
    if (item.mesh.position.z > AREA + 2) {
