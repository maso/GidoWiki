...
const BUBBLE_HOVER_BASE  = 5;     // s on first respawn
const BUBBLE_HOVER_INC   = 3;     // s added per additional respawn
...
const _BB_Z_BOT =  DAMAGE_Z - BODY_RADIUS;       //  8.48 (just above fire line)
...
function _activateBubble(pi) {
  ...
  // Random spawn point within radius 5 of center, within bounds
  const angle  = Math.random() * Math.PI * 2;
  const radius = Math.sqrt(Math.random()) * 5;
  const spawnX = clamp(Math.cos(angle) * radius, -_BB_X, _BB_X);
  const spawnZ = clamp(Math.sin(angle) * radius, _BB_Z_TOP, _BB_Z_BOT);

  b.active       = true;
  b.phase        = 'descending';
  b.phaseTimer   = 0;
  b.hoverDur     = BUBBLE_HOVER_BASE + Math.max(0, p.deathCount - 1) * BUBBLE_HOVER_INC;
  b.x            = spawnX;
  b.z            = spawnZ;
  b.targetX      = spawnX;
  b.targetZ      = spawnZ;
...
function updateBubbles(dt) {
  ...
  bubbles.forEach((b, pi) => {
    ...
    if (p.isDead && !p.inBubble && !b.active &&
        p.deathTimer >= BUBBLE_SPAWN_DELAY) {
      _activateBubble(pi);
      return;
    }
    ...
    if (b.phase === 'descending') {
      // Player can already steer horizontally during descent
      _applyBubbleInput(pi, dt);
      const lerpK = 1 - Math.pow(0.001, dt);
      b.x += (b.targetX - b.x) * lerpK;
      b.z += (b.targetZ - b.z) * lerpK;
      ...
      // Input moves the invisible target; bubble follows with lag
      _applyBubbleInput(pi, dt);
      const lerpK = 1 - Math.pow(0.001, dt);
      b.x += (b.targetX - b.x) * lerpK;
      b.z += (b.targetZ - b.z) * lerpK;

      // ── Struggle animation tick ────────────────────────────────────────
      if (b.struggleTimer > 0) {
        b.struggleTimer -= dt;
        const st = BUBBLE_STRUGGLE_DUR - Math.max(0, b.struggleTimer);
        const shakeA = Math.sin(st * Math.PI * 2 * 10);
        const shakeB = Math.sin(st * Math.PI * 2 * 9 + 1.2);
        ...
        if (b.struggleTimer <= 0) b.struggleTimer = 0;
      }

      // ── Clamp timer so label always reads 0 once expired ─────────────
      if (b.phaseTimer >= b.hoverDur) {
        b.phaseTimer = b.hoverDur;
        b.hp = Math.min(b.hp, 1); // ensure one press is enough to escape
      }

      const remaining = Math.max(0, b.hoverDur - b.phaseTimer);
      _drawLabel(b, Math.ceil(remaining));

      // ── Attack input → trigger struggle (only when landing is safe) ───
      landingOk = _isLandingFree(b.x, b.z, pi);

      if (b.struggleTimer <= 0) {
        const _bnd = playerBindings[pi];
        // Always keep gamepad state in sync
        let btn = false;
        if (_bnd && _bnd.type === 'gamepad') {
          const gp = navigator.getGamepads()[_bnd.gpIndex];
          btn = gp ? !!gp.buttons[2]?.pressed : false;
        }

        if (landingOk) {
          let attacked = false;
          if (_bnd && _bnd.type === 'keyboard') {
            attacked = attackPressed;
          } else {
            attacked = btn && !b.gpAttackPrev;
          }

          if (attacked) {
            b.struggleTimer = BUBBLE_STRUGGLE_DUR;
            b.hp -= 1;
            if (b.hp <= 0) {
              // Pop — deduct any remaining countdown from the game timer
              if (remaining > 0) {
                timeRemaining = Math.max(0, timeRemaining - remaining);
                document.getElementById('timer-display').textContent = Math.ceil(timeRemaining);
                showTimerPenalty(remaining);
              }
              _popBubble(b, p);
              return;
            }
        ...
        if (_bnd && _bnd.type === 'gamepad') b.gpAttackPrev = btn;
      } else {
        // Keep gpAttackPrev in sync even when locked
        const _bnd = playerBindings[pi];
        if (_bnd && _bnd.type === 'gamepad') {
          const gp = navigator.getGamepads()[_bnd.gpIndex];
          b.gpAttackPrev = gp ? !!gp.buttons[2]?.pressed : false;
        }
      }

    } else if (b.phase === 'falling') {
      // Player can nudge horizontal position during fall
      _applyBubbleInput(pi, dt);
