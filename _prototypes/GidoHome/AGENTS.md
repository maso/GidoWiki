...
- **表情輪盤**：`src/emote/`，只在主畫面生效（三個面板任一開啟時會被抑制）。
  手把推右搖桿開啟、停留 0.5 秒選取；鍵鼠按住左 Shift 開啟、滑鼠瞄準後點擊或放開
  Shift 選取。選取後 0.3 秒關閉，並在隨機一隻非蛋角色頭上冒出表情泡泡 2.4 秒。
  六個表情放在 `emoteData.js` 方便替換；輪盤幾何會依陣列長度自動計算。
- **測試規範**：任何修改都需確認 `npm test` 通過 39 項單元測試。
